...
class LogcatVTime(QObject):
    def __init__(self, fileName):
        self.mFileName = fileName
        self.mStopWorking = False
        super(LogcatVTime,self).__init__()
    ...
    def identify(self):
        print "LogcatVTime" + self.mFileName
        ctr = 0
        passCtr = 0
        itr = 0
        ...
        with open(self.mFileName,'rb') as f:
            for line in f:
                line = re.sub('[\r\n]','',line)
                s1 =  re.findall(r'\d{2}-\d{2} \d{2}:\d{2}:\d{2}.\d{3} [a-zA-Z]/.*[\s]*\([\s]*\d*\): .*',line)
                tmpstr = ""
                if len(s1) > 0:
                    tmpstr = s1[0]
                    tmpstr = re.sub('[\r\n]','',tmpstr)
        ...
        if passCtr >= 7:
            print "LogcatVTime TRUE"
            return True
        else:
            print "LogcatVTime FALSE"
            return False
